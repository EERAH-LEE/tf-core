...
resource "aws_instance" "this" {
  ...
  #depends_on = [aws_iam_role_policy_attachment.this] 모듈로서 의존성이 생겨서 필요없음.

  tags = {
    Name = "${local.namespace}-intance-${local.instance.name}"
  }
}

resource "aws_lb_target_group_attachment" "this" {
  target_group_arn = local.lb.aws_lb_target_group_arn
